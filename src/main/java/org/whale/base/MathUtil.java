...
public class MathUtil implements Serializable {
    ...
    /** 
     * 提供精确的乘法运算。 精确到小数点以后2位，以后的数字向下取整，直接舍掉。
     * 不足一分钱的算一分钱
     *  
     * @param value1 
     *            被乘数 
     ...
     * @return 两个参数的积 
     */  
    public static BigDecimal mulCal2Down(BigDecimal value1, BigDecimal value2) {
    	if(value1 == null || value2 == null) {
			throw new BusinessException("调用MathUtil.mul()参数为null");
    	}
    	
        BigDecimal result = value1.multiply(value2).setScale(SCALE_2,BigDecimal.ROUND_DOWN);
        if(MathUtil.isEquals(result, BigDecimal.ZERO)){
    ...
    /** 
     * 提供精确的乘法运算。 精确到小数点以后2位，以后的数字向上取整。
     *  
     * @param value1 
     *            被乘数 
     * @param value2 
     ...
     * @return 两个参数的积 
     */  
    public static BigDecimal mulCal2Up(BigDecimal value1, BigDecimal value2) {
    	if(value1 == null || value2 == null) {
			throw new BusinessException("调用MathUtil.mul()参数为null");
    	}
    	
        return value1.multiply(value2).setScale(SCALE_2,BigDecimal.ROUND_UP);
    }
    
    /** 
     * 提供int 类型的除法运算，当发生除不尽的情况时，向上取整。 
     *  
     * @param dividend 
     *            被除数 
     * @param divisor 
     *            除数 
     * @return 两个参数的商，向上取整。比如7/3=2.3，返回3。
     */ 
	public static int divCeilToUp(int dividend, int divisor) {
		return roundUp(divCal10(new BigDecimal(dividend), new BigDecimal(divisor)), 0).intValue();
	}
    ...
    public static BigDecimal roundUp(BigDecimal value, Integer scale) {
    	if(value == null) {
			throw new BusinessException("调用MathUtil.roundUp()参数为null");
    	}
        if (scale < 0) {  
            throw new IllegalArgumentException(  
                    "The scale must be a positive integer or zero");  
        ...
        BigDecimal one = new BigDecimal("1");  
        return value.divide(one, scale, BigDecimal.ROUND_UP);  
    }
...
//		BigDecimal b1 = new BigDecimal(1000000.0000);
//		BigDecimal b2 = new BigDecimal(9);
//		BigDecimal b3 = new BigDecimal(11);
//		System.out.println(MathUtil.percentWToDecimal(b1));
//		System.out.println(MathUtil.isGreaterThan(b1, b2));
//		System.out.println(MathUtil.isGreaterThanOrEqual(b1, b2));
//		System.out.println(MathUtil.percentWToDecimal(new BigDecimal(12)));
...
//		System.out.println(divCeilToUp(90, 30));
//		System.out.println(divCeilToUp(1, 30));
//		BigDecimal tt = new BigDecimal(0.00);
//		System.out.println(MathUtil.isEquals(tt, BigDecimal.ZERO));
		BigDecimal a1 = new BigDecimal(0.05);
		BigDecimal a2 = new BigDecimal(0.1);
		System.out.println(MathUtil.mulCal2Up(a1, a2));
		System.out.println(MathUtil.mulCal2Down(a1, a2));
	}
