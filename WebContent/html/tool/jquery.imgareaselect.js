...
$.imgAreaSelect = function (img, options) {
    ...
    function zooming(event) {
    	...
    	var newWidth = 0;
    	var newHeight = 0;
    	if(typeof(event.data.width) == "undefined" || event.data.width == 0){
	    	newWidth = Math.round($img.width() * (10+event.data.stepX) / 10);
	    	newHeight = Math.round($img.height() * (10+event.data.stepY) / 10);
    	}else{
