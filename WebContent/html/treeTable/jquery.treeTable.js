...
(function ($) {
    ...
    $.fn.treeTable = function (opts) {
        ...
        $treeTable.click(function (event) {
            ...
            if ($target.parents('tr').attr('controller')) {
            ...
            if ($target.attr('arrow')) {
                ...
                if (className == css['AN'] + ' ' + css['HLO'] || className == css['AN'] + ' ' + css['HO']) {
                   	 var id = "";
                   	try{
	                   id = $target.parents('tr[haschild]')[0].id;
	                  
                    }catch(error){
                    	id = $target.parents('tr')[0].id;
                    }
        ...
        function formatNode(tr) {
            ...
            if(opts.async&&$cur.attr("length")!=null&&$cur.attr("length")>0){
            	 var className = $cur.attr('isLastOne') ? css['LS'] : css['S'];
           		 className = css['AN'] + ' ' + className;
           		 $td.attr("style",$td.attr("style")!=null&&$td.attr("style")!="undefined"?$td.attr("style"):''+"font-weight: bold;");
            }else{
